...
- 全 20 ステージ 600 問（1 ステージ 30 問）。CEFR C1 レベルまでの文法をカバー
- 和訳は解答前から表示され、入力すると即座に正誤フィードバック（正解・不正解どちらでも解説を表示）
- ステージは 5 カテゴリに分かれ、選択画面でグループ表示

| カテゴリ | ステージ |
|---|---|
| 前置詞 | 基礎 2（時間・場所 / 方向・手段）＋ 応用 4（抽象への拡張 / 動詞 / 形容詞・名詞 / 紛らわしいペアと群前置詞） |
| 冠詞 | a・an・the / the と無冠詞 |
| 名詞 | 数量表現 / 可算・不可算と複数形 |
| 時制 | 現在 / 過去 / 現在完了 / 過去完了・未来完了 / 未来 / 受動態 / 仮定法・時制の一致 |
| 場面別 | ビジネスの前置詞 / 冠詞と不可算名詞 / メールの時制と語法 |

- ステージ完了後に正答率を表示し、ステージ選択画面に前回の正答率を表示
- 復習セッション: 完了後に間違えた問題だけを解き直せる。**復習の正解は正答率にも学習記録にも
  含まれない**（設定画面でオンオフ可能、既定はオン）
- 学習記録: 日ごとにクリアしたステージ数を月間カレンダーのヒートマップで振り返り。
  トップ画面にも直近 7 日分（今日が右端）の帯を表示
