...
| `grammar-train:settings:v1` | 設定（`{ reviewEnabled: boolean }`）。既定は復習オン |
...
ステージ完了後、結果画面の「間違えた問題を復習」から、間違えた問題だけをもう一度出題する。

- **復習は記録に一切残さない。** 正答率（`records`）も学習記録のクリア数（`history`）も
  更新しない。これは `App.tsx` で `handleFinish`（通常）と `handleReviewFinish`（復習）を
  **別の関数に分けて**担保している。フラグで分岐させると必ず取り違えるので、統合しないこと
- 復習は**1 周だけ**。復習中にまた間違えても繰り返さない。2 回目の復習も「元の」間違いリストを
  そのまま出す（`SessionResult.wrong` は書き換えない）
- **間違いリストは永続化しない**（React の state だけ）。`Question` に id はなく、問題文は
  改訂されうるので、保存しても次のデプロイで指す先が消える。`Question.id` を足したくなったら
  この判断ごと考え直すこと
- `Quiz` の `reviewQuestions` に**空配列を渡すと落ちる**。0 問なら結果画面が復習ボタンを
  出さない、で担保している
- `Quiz` の描画箇所は通常用と復習用の 2 か所ある。1 つにまとめると同じスロット・同じ key に
  なり、出題位置や判定が持ち越される（`App.test.tsx` の「復習を2回続けて始めても1問目から
  始まる」が退行を検出する）
- 空欄ヒント（`hasBlankAnswer`）は**必ず `stage.questions` 全体から判定する**。出題中の配列で
  判定すると、答えが空欄の 1 問だけを復習したときに答えそのものが漏れる
- オンオフは設定画面（`grammar-train:settings:v1` の `reviewEnabled`、既定オン）
